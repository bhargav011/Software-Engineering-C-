...
CArray::CArray(unsigned int Number_of_Elements)
{
    this -> Builder(Number_of_Elements);
}
...
void CArray::Builder()
{
    ...
    this -> Number_of_Elements = a;

    this -> Array = new int [this -> Number_of_Elements];

    for(unsigned int i = 0; i < this -> Number_of_Elements; i++)
    {
        this -> Array[i] = rand() % 10; // Randomly assign values for array
    }
}

// Initializes the size of the Array which is given by user
void CArray::Builder(unsigned int Number_of_Elements)
{
    this -> Number_of_Elements = Number_of_Elements;

    this -> Array = new int[Number_of_Elements];
...
void CArray::Display() const
{
    ...
    for(unsigned int i = 0; i < this->Number_of_Elements; i++)
    {
        cout << "Value at Index " << i << " is " <<  this -> Array[i] << endl;
    }
    cout << endl;
}
...
{
    int Bubble = 0;
    Bubble = this -> Array [Index1];
    this -> Array[Index1] = this -> Array[Index2];
    this -> Array[Index2] = Bubble;
}
...
void CArray::BubbleSort()
{
    for(unsigned int i = 0; i < this -> Number_of_Elements; i++)
    {
        for(unsigned int j = i + 1; j < this -> Number_of_Elements; j++)
        {
...
{
    this -> Recursively_Sort(this -> Array, 0, this -> Number_of_Elements - 1);
...
void CArray::SelectionSort()
{
    ...
    for(unsigned int i = 0; i < this -> Number_of_Elements; i++)
    {
        ...
        for(unsigned int j = i + 1; j < this -> Number_of_Elements; j++)
            if(this -> Array[j] < this -> Array[Sorted_Part]) // Finds the smallest element
               Sorted_Part = j;
...
void CArray::InsertionSort()
{
    ...
    for(unsigned int i = 1; i < this -> Number_of_Elements; i++)
    {
        Insert = this -> Array[i]; // Value will be inserted into the array
        Sorted_null = i; // position i as the null Index

        while(Sorted_null > 0 && Insert < this -> Array[Sorted_null -1])
        {
            // Shift the larger value up
            this -> Array[Sorted_null] = this -> Array[Sorted_null - 1];
            Sorted_null--;
        }
        // Inserts it into the right slot of the sorted array
        this -> Array[Sorted_null] = Insert;
    }
}
...
void CArray::BinarySort()
{
    ...
    root = Insert_value(root, this -> Array[0]);

    for (unsigned int i=0; i < this -> Number_of_Elements; i++)
        Insert_value(root, this -> Array[0]);

    // Store inoder traversal of the BST
    int n = 0;
    Store_value(root, this -> Array, n);
}
